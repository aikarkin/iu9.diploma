...
Програмное решение, предназначено для решения задачи автоматического составления расписания занятий в университете с помощью SMT солвера. Более подробное описание дипломной работы можно найти в [расчетно пояснительной записке](./docs/IU9-82_KarkinAI_rpz.pdf). Данное решение соститоит из базы данных, в которой хранится вспомогательная информация для составления расписания, а также из ряда утилит для гереации расписания и формирования pdf-документа с раписанием занятий группы.
...
* **schedule-parser** - вспомогательный модуль, содержит классы для парсинга csv-файлов, а также парсинга html-страниц с [расписанием](https://students.bmstu.ru/schedule/list/). Парсинг html-страниц используется для сбора вспомогательной информации, которая нужна на этапе составления расписания. Парсинг csv-файлов используется для реализации утилиты dbfill.

По-мимо папки с модулями, в данном репозитории можно найти следующие директории:

* ./build - готовые jar-приложения (утилита _smtgen_ может не работать на разных архитектурах из-за библиотеки Z3, поэтому может понадобиться пересборка данного модуля).
...
> Данный проект не рекомендуется запускать из-под ОС Windows. Несмотря на то, что утилиты после сборки в данной ОС работают корректно, возможны проблемы при вводе русских символов из консоли.
...
1. Установить Postgres с помощью команды в терминале:

```console
$ sudo apt-get install postgresql postgresql-contrib
```

2. Затем, создаем пользователя в базе (рекомендуемое имя пользователя admin):
...
3. Запустить скрипт для создания базы данных:
...
После этого необходимо пересобрать модуль _dbfill_ ( отом как это сделать говорится в следующем разделе), после чего в директории модуля выполнить команду:

```console
$ java -jar ./target/dbfill.jar
```

Если реквизиты пользователя в пункте 2) указывались как в примере, то следующий раздел можно пропустить.

# Сборка проекта

Для сборки всего проекта понадибиться установить следующее:
...
О том как это сделать под ОС Windows рассказывается на странице репозитория [Z3](https://github.com/Z3Prover/z3).

Для сборки модуля _smtgen_ под ОС Linux понадобиться выполнить следующие шаги:
...
3. Установить собранные java-байндинги в локальном репозитории Maven, выполнив команду в терминале в той же директории:
...
Для демонстрации работы программы использулся конфигурационный файл - [config.properties](./dbfill/src/main/resources/config.properties). В данном файле приводятся пути к csv-справочникам, содержащими данные для кафедры ИУ9. Этот же файл используется для заполнения данными базы при первом запуске docker-контейнера (см. раздел [Установка и настройка базы](#установка-и-настройка-базы), пункт а).

Результатом успешного завершения данной утилиты будет заполненная база данных.

## Утилита smtgen

Утилита выполняет автоматическое составление раписания. На вход данная утилита принимает, в качестве, аргументов командной одно из двух:
...
Программа, в качестве входных параметров, принимает следующие аргументы командной в заданном порядке:
